...
class BookingSerializer(serializers.ModelSerializer):
    ...
    flight_no = serializers.PrimaryKeyRelatedField(queryset=Flight.objects.all())
    ...
    def validate(self, data):
        ...
        current_booked_seats = (
            Booking.objects.filter(
                flight_no=flight_no, booking_class=booking_class, date=date
            )
            .exclude(pnr=data["pnr"])
            .aggregate(Sum("seats_booked"))
            .get("seats_booked__sum", 0)
            or 0
        )

        if (
            getattr(flight_no, self.classes.get(booking_class))
            < no_of_seats + current_booked_seats
        ):
    ...
    def update(self, old_data, new_data):
        ...
        new_flight_no = new_data.get("flight_no")
        ...
        current_booked_seats = (
            Booking.objects.filter(
                flight_no=new_flight_no, booking_class=new_booking_class, date=date
            )
            .exclude(pnr=old_data.pnr)
            .aggregate(Sum("seats_booked"))
            .get("seats_booked__sum", 0)
            or 0
        )
        if (
            getattr(new_flight_no, self.classes.get(new_booking_class))
            < no_of_seats + current_booked_seats
        ):
            # raise error if not
